...
Styles: 3Blue1Brown / THEMATHFLOW inspired math education visualization.
Themes: color (default), mono (minimal black/white)

Features:
- Clean geometric shapes
- Low-saturation color palette (color) or grayscale (mono)
...
MATH_THEME_COLOR = {
    ...
    "grid": "#e0dbd5",
    "text": "#2c2c2c",
    "accent1": "#e8734a",   # unit circle
    "accent2": "#d4567a",   # sin
    "accent3": "#4a9e8e",   # cos
    "accent4": "#6b7b8d",   # secondary
    "highlight": "#f0c040",
}
...
THEMES = {
    "color": MATH_THEME_COLOR,
    "mono": MATH_THEME_MONO,
}
...
def draw_grid(draw, width, height, theme, spacing=50):
    color = theme["grid"]
...
def draw_sin_curve(draw, start_x, y_center, width, height, angle, theme):
    ...
    return start_x + (angle / (2 * math.pi)) * width, y_center - math.sin(angle) * height * 0.4
...
def draw_cos_curve(draw, start_x, y_center, width, height, angle, theme):
    ...
    return start_x + (angle / (2 * math.pi)) * width, y_center - math.cos(angle) * height * 0.4
...
def draw_projection_lines(draw, px, py, sin_x, sin_y, cos_x, cos_y, theme):
    ...
    for i in range(0, 100, 8):
        ...
        draw.line(
            [
                (c(px + (sin_x - px) * t1), c(py + (sin_y - py) * t1)),
                (c(px + (sin_x - px) * t2), c(py + (sin_y - py) * t2)),
            ],
    ...
    for i in range(0, 100, 8):
        ...
        draw.line(
            [
                (c(px + (cos_x - px) * t1), c(py + (cos_y - py) * t1)),
                (c(px + (cos_x - px) * t2), c(py + (cos_y - py) * t2)),
            ],
...
def render_frame(data, progress, width, height, theme):
    img = Image.new("RGBA", (width * SCALE, height * SCALE), hex_rgba(theme["bg"]))
    draw = ImageDraw.Draw(img)

    draw_grid(draw, width, height, theme)
    ...
    # Unit circle
    circle_cx, circle_cy, circle_radius = 200, 350, 120
    px, py = draw_unit_circle(draw, circle_cx, circle_cy, circle_radius, angle, theme)

    # Sin curve
    sin_start_x, sin_y, sin_width, sin_height = 400, 250, 700, 200
    sin_cx, sin_cy = draw_sin_curve(draw, sin_start_x, sin_y, sin_width, sin_height, angle, theme)

    # Cos curve
    cos_start_x, cos_y, cos_width, cos_height = 400, 550, 700, 200
    cos_cx, cos_cy = draw_cos_curve(draw, cos_start_x, cos_y, cos_width, cos_height, angle, theme)

    # Projection lines
    draw_projection_lines(draw, px, py, sin_cx, sin_cy, cos_cx, cos_cy, theme)
...
def render_math_animation(data, outdir, basename, frames=DEFAULT_FRAMES, fps=DEFAULT_FPS):
    ...
    theme_name = data.get("theme", "color")
    theme = THEMES.get(theme_name, MATH_THEME_COLOR)
    ...
    for i in range(frames):
        progress = i / (frames - 1)
        frame = render_frame(data, progress, width, height, theme)
        images.append(frame)
...
def main():
    ...
    parser.add_argument("--theme", default=None, help="Theme: color or mono (default: color)")
